...
class RequestFileStrategy
{
    ...
    {
        return $this->getFileRequests($tags);
    }
     ...
     * @return array|null
     */
    protected function getFileRequests(array $tags)
    {
        ...
        $requestFileTags = array_values(
            array_filter($tags, function ($tag) {
                return $tag instanceof Tag && strtolower($tag->getName()) === 'requestfile';
            })
        );

        if (empty($requestFileTags)) {
            return;
        }

        return array_map(function (Tag $requestFileTag) {
            preg_match('/^(\d{3})?\s?([\S]*[\s]*?)(\{.*\})?$/', $requestFileTag->getContent(), $result);
            $status = $result[1] ?: 200;
