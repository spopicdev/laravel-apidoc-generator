...
use Mpociot\ApiDoc\Tools\ResponseStrategies\RequestFileStrategy;
...
class RequestResolver
{
        ...
        foreach (static::$strategies as $strategy) {
            ...
            if (! is_null($responses)) {
                return array_map(function (Response $response) {
                    return ['status' => $response->getStatusCode(), 'content' => $this->getRequestContent($response)];
                }, $responses);
            }
        }
    }
     ...
     * @return mixed
     */
    private function getRequestContent($response)
    {
